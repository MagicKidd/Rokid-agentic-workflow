...
def _resolve_scan_roots(root: Path) -> tuple[list[Path], list[Path]]:
    """
    Prefer common code roots while staying project-agnostic.
    """
    src_roots: list[Path] = []
...
_FRAMEWORK_CLASSES = frozenset({"FastAPI", "APIRouter", "Compiler", "ArgumentParser", "Counter", "Path"})
...
- **Docstrings**: {doc_lang}, Google-style sections (Args/Returns) count={scan.docstring_google}
...
def _write_project_context(root: Path, out: Path, scan: ScanResult) -> None:
    ...
    manual = _extract_manual_notes(
        out,
        default_text="在这里记录业务语义、架构决策和约束（人工维护）。\n自动刷新不会覆盖本区域内容。",
    )
...
1. 用户明确指示
2. 本文件约定（项目级）
3. 全局行为基线（如 `ai-coding-protocol.mdc`）

## 约定摘要（可执行）

{singleton}- **Type hints**: {type_hint}
- **Naming**: PascalCase classes, snake_case functions, UPPER_SNAKE constants
- **Logging**: prefer module-level `logger = Logger(__name__).get_logger()` when available
- **Docs/Comments**: follow dominant language in repo (see `.agent-context/conventions.md`)

数据来源：{len(scan.src_files)} 源文件 + {len(scan.test_files)} 测试文件。详见 `.agent-context/conventions.md`。
"""
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(content, encoding="utf-8")
